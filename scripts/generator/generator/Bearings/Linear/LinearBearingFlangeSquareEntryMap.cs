...
{
    internal sealed class LinearBearingFlangeSquareEntryMap : CsvClassMap<LinearBearingEntry>
    {
        public LinearBearingFlangeSquareEntryMap()
        {
            Map(m => m.Model)
                .Index(0);
            Map(m => m.BallRows)
            ...
            Map(m => m.FlangeMountingSize)
                .TypeConverter(new MountHoleConverter(0)).TypeConverterOption(CultureInfo.InvariantCulture).Index(13);
            Map(m => m.FlangeMountingHeadSize)
                .TypeConverter(new MountHoleConverter(1)).TypeConverterOption(CultureInfo.InvariantCulture).Index(13);
            Map(m => m.FlangeMountingHeadDepth)
                .TypeConverter(new MountHoleConverter(2)).TypeConverterOption(CultureInfo.InvariantCulture).Index(13);
